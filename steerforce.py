...
class SteeringFeedback:
    ...
    def loop(self, _):        
        ...
        t_corr, atk_len = self.getTorque(p_act, self.p_des, self.k_des)

        self.setTorque(t_corr, atk_len)

    def getTorque(self, p_des, p_act, k):
        p_err = p_des - p_act      
        print(p_err)  
        ...
        if abs(p_err) > DEVICE.TOLERANCE:
            atk_len = 0.005 #DEVICE.TIME_STEP #/ 10
            # atk_len = DEVICE.TIME_STEP #/ 10

            mul = 1 if p_err >= 0 else -1

            t_des = k * (p_err)
            t_corr = -1* mul * limit(abs(t_des), DEVICE.MIN_TORQUE, DEVICE.MAX_TORQUE)
                    
        return t_corr, atk_len
    ...
    def setTorque(self, t_norm, atk_len):
        ...
        # self.effect.u.ff_constant_effect.ff_envelope.fade_level = int(0x7FFF * abs(t_norm))
